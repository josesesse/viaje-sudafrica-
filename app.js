...
// Abre el Copiloto desde la barra de búsqueda. Si hay una pregunta, la
// precarga y la envía automáticamente; si no, simplemente abre el panel.
function openCopilotoWithQuery(query){
  if(!window.Copiloto){
    console.error("Copiloto no está disponible todavía.");
    return;
  }
  window.Copiloto.abrir(query && query.trim() ? query.trim() : null);
}
...
function renderDayList(){
  ...
  if(days.length===0){
    list.innerHTML = `<div class="empty">
      ${icon("search")}
      <p>No hay ningún día que coincida con la búsqueda.</p>
      ${currentQuery ? `<button class="empty-ask-copiloto" id="ask-copiloto-btn" type="button">${icon("chat")}Preguntar al Copiloto</button>` : ""}
    </div>`;
    const askBtn = document.getElementById("ask-copiloto-btn");
    if(askBtn) askBtn.addEventListener("click", ()=> openCopilotoWithQuery(currentQuery));
    return;
  }
  let html = "";
  let lastLeg = null;
...
function init(){
  ...
  document.getElementById("search").addEventListener("keydown", (e)=>{
    ...
    if(!q) return;
    const matches = TRIP.days.filter(matchesFilter);
    if(matches.length === 0){
      e.preventDefault();
      openCopilotoWithQuery(q);
    }
  });

  const searchCopilotoBtn = document.getElementById("search-copiloto-btn");
  if(searchCopilotoBtn){
    searchCopilotoBtn.addEventListener("click", ()=> openCopilotoWithQuery(currentQuery));
  }
